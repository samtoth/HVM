...
#define I32 (0xC) // arity = 0 (unboxed)
#define F32 (0xD) // arity = 0 (unboxed)
...
Lnk I_32(i32 val) {
  return (I32 * TAG) | (u64)*(uint32_t*)(void*)(&val);
}

Lnk F_32(f32 val) {
  return (F32 * TAG) | (u64)*(uint32_t*)(void*)(&val);
}
      ...
      switch (get_tag(term)) {
        ...
        case OP2: {
          ...
          if (get_tag(arg0) == U32 && get_tag(arg1) == U32) {
            ...
            u64 c = 0;
            switch (get_ext(term)) {
              case ADD: c = I_32(a +  b);                break;
              case SUB: c = I_32(a -  b);                break;
              case MUL: c = I_32(a *  b);                break;
              case DIV: c = I_32(a /  b);                break;
              case MOD: c = I_32(a %  b);                break;
              case AND: c = I_32(a &  b);                break;
              case OR : c = I_32(a |  b);                break;
              case XOR: c = I_32(a ^  b);                break;
              case SHL: c = I_32(a << b);                break;
              case SHR: c = I_32(a >> b);                break;
              case LTN: c = U_32((a <  b) ? 1 : 0);      break;
              case LTE: c = U_32((a <= b) ? 1 : 0);      break;
              case EQL: c = U_32((a == b) ? 1 : 0);      break;
              case GTE: c = U_32((a >= b) ? 1 : 0);      break;
              case GTN: c = U_32((a >  b) ? 1 : 0);      break;
              case NEQ: c = U_32((a != b) ? 1 : 0);      break;
            }
            clear(mem, get_loc(term,0), 2);
            link(mem, host, c);
          }
            ...
            u64 c = 0;
            switch (get_ext(term)) {
              case ADD: c = F_32(a +  b);                break;
              case SUB: c = F_32(a -  b);                break;
              case MUL: c = F_32(a *  b);                break;
              case DIV: c = F_32(a /  b);                break;
              case MOD: c = F_32(a %  b);                break;
              case AND: c = F_32(a &  b);                break;
              case OR : c = F_32(a |  b);                break;
              case XOR: c = F_32(a ^  b);                break;
              case SHL: c = F_32(a << b);                break;
              case SHR: c = F_32(a >> b);                break;
              case LTN: c = U_32((a <  b) ? 1 : 0);      break;
              case LTE: c = U_32((a <= b) ? 1 : 0);      break;
              case EQL: c = U_32((a == b) ? 1 : 0);      break;
              case GTE: c = U_32((a >= b) ? 1 : 0);      break;
...
void readback_signed(Stk* chrs, i32 n) {
  if (n < 0) {
    stk_push(chrs, '-');
    readback_decimal(chrs, -n);
  } else {
    readback_decimal(chrs, n);
  }
}
...
void readback_term(Stk* chrs, Worker* mem, Lnk term, Stk* vars, Stk* dirs, char** id_to_name_data, u64 id_to_name_mcap) {
  ...
  switch (get_tag(term)) {
    ...
    case I32: {
      readback_signed(chrs, get_i32(term));
      break;
    }
    case F32: {
      readback_floating(chrs, get_f32(term));
      break;
    }
    case CTR: case CAL: {
      u64 func = get_ext(term);
